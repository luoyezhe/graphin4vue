...
import { setStatusStyle, removeDumpAttrs, convertSizeToWH, getRectLabelXYByPosition, getBadgePosition } from './utils'
...
const parseHalo = (style) => {
  ...
  let keyshapeFill
  let keyshapeStroke
  if (keyshape && keyshape.width && keyshape.height) {
    haloWidth = keyshape.width + 10
    haloHeight = keyshape.height + 10
  }
  ...
  if (keyshape && keyshape.radius) {
    haloRadius = keyshape.radius
  }
...
const parseLeftShape = (style) => {
  const {
    keyshape,
    leftShape
  } = style
  const { height } = keyshape
  const {
    radius,
    stroke,
    fill,
...
const parseLabel = (style) => {
  ...
  const labelPos = getRectLabelXYByPosition(style, true)
  const attrs = {
    x: labelPos.x,
    y: labelPos.y,
    fontSize,
...
export default () => {
  G6.registerNode('graphin-rect', {
    ...
    draw (cfg, group) {
      ...
      // halo 光晕
      group.addShape('rect', parseHalo(style))
      // keyshape 主容器
      const keyShape = group.addShape('rect', parseKeyshape(style))
      group.addShape('rect', parseLeftShape(style))
      // 文本
